...
original_text=[r'C:\Users\Lenovo\Desktop\测试集\orig.txt',r'C:\Users\Lenovo\Desktop\测试集\短orig.txt','666','777','888','999']
...
class MyTestCase(unittest.TestCase):
    ...
    @patch('builtins.input')
    def test_short_text_preprocess(self,mock_input):
        result = DuplicateChecking()
    ...
    def test_text_checking(self, mock_input):
        result = DuplicateChecking()
        mock_input.side_effect = [original_text[0], test_text[random.randint(0, 4)],r'C:\Users\Lenovo\Desktop\测试集\查重数据.txt']#正确的输入
        self.assertEqual(result.text_checking(), True)

        mock_input.side_effect = [original_text[1], test_text[5],r'C:\Users\Lenovo\Desktop\测试集\查重数据.txt']  # 正确的输入
        self.assertEqual(result.text_checking(), True)

        mock_input.side_effect = [original_text[random.randint(2,5)], test_text[random.randint(6, 9)],r'C:\Users\Lenovo\Desktop\测试集\查重数据.txt']# 正确的输入
